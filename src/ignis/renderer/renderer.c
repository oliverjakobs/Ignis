...
{
    if (ignisGenerateVertexArray(&quad->vao, 2) != IGNIS_SUCCESS) return IGNIS_FAILURE;

    GLsizeiptr size = sizeof(GLfloat) * vertex_count;
    if (ignisLoadArrayBuffer(&quad->vao, 0, size, vertices, usage) != IGNIS_SUCCESS)
        return IGNIS_FAILURE;

    ignisSetVertexLayout(&quad->vao, 0, layout, layout_size);
    quad->vertex_count = vertex_count;

    if (ignisLoadElementBuffer(&quad->vao, 1, indices, element_count, GL_STATIC_DRAW) != IGNIS_SUCCESS)
        return IGNIS_FAILURE;

    quad->element_count = element_count;
